...
class ColPaliModel:
    ...
    def add_to_index(
        ...
        if doc_ids and len(doc_ids) != len(input_items):
            print(f"Number of doc_ids ({len(doc_ids)}) does not match number of input items ({len(input_items)})")
        ...
        if metadata and len(metadata) != len(input_items):
            print(f"Number of metadata entries ({len(metadata)}) does not match number of input items ({len(input_items)})")
